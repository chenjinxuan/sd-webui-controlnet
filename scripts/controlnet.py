...
class Script(scripts.Script):
    ...
    def uigroup(self, tabname, is_img2img):
        ...
        with gr.Row():
            input_image = gr.Image(elem_id='ext_ctl_image',source='upload', mirror_webcam=False, type='numpy', tool='sketch')
        ...
        with gr.Row():
            enabled = gr.Checkbox(elem_id='ext_ctl_enabled',label='Enable', value=default_unit.enabled)
            scribble_mode = gr.Checkbox(elem_id='ext_ctl_scribble_mode',label='Invert Input Color', value=default_unit.invert_image)
            rgbbgr_mode = gr.Checkbox(elem_id='ext_ctl_rgbbgr_mode',label='RGB to BGR', value=default_unit.rgbbgr_mode)
            lowvram = gr.Checkbox(elem_id='ext_ctl_lowvram',label='Low VRAM', value=default_unit.low_vram)
            guess_mode = gr.Checkbox(elem_id='ext_ctl_guess_mode',label='Guess Mode', value=default_unit.guess_mode)
        ...
        with gr.Row():
            module = gr.Dropdown(list(self.preprocessor.keys()),elem_id='ext_ctl_module', label=f"Preprocessor", value=default_unit.module)
            model = gr.Dropdown(list(global_state.cn_models.keys()), elem_id='ext_ctl_model',label=f"Model", value=default_unit.model)
            refresh_models = ToolButton(value=refresh_symbol)
            refresh_models.click(refresh_all_models, model, model)
                # ctrls += (refresh_models, )
        with gr.Row():
            weight = gr.Slider(elem_id='ext_ctl_weight',label=f"Weight", value=default_unit.weight, minimum=0.0, maximum=2.0, step=.05)
            guidance_start = gr.Slider(elem_id='ext_ctl_guidance_start',label="Guidance Start (T)", value=default_unit.guidance_start, minimum=0.0, maximum=1.0, interactive=True)
            guidance_end = gr.Slider(elem_id='ext_ctl_guidance_end',label="Guidance End (T)", value=default_unit.guidance_end, minimum=0.0, maximum=1.0, interactive=True)
        ...
        with advanced:
            processor_res = gr.Slider(elem_id='ext_ctl_processor_res',label="Annotator resolution", value=default_unit.processor_res, minimum=64, maximum=2048, interactive=False)
            threshold_a =  gr.Slider(elem_id='ext_ctl_threshold_a',label="Threshold A", value=default_unit.threshold_a, minimum=64, maximum=1024, interactive=False)
            threshold_b =  gr.Slider(elem_id='ext_ctl_threshold_b',label="Threshold B", value=default_unit.threshold_b, minimum=64, maximum=1024, interactive=False)
        ...
        resize_mode = gr.Radio(elem_id='ext_ctl_resize_mode',choices=[e.value for e in external_code.ResizeMode], value=default_unit.resize_mode.value, label="Resize Mode")
        with gr.Row():
            with gr.Column():
                canvas_width = gr.Slider(elem_id='ext_ctl_canvas_width',label="Canvas Width", minimum=256, maximum=1024, value=512, step=64)
                canvas_height = gr.Slider(elem_id='ext_ctl_canvas_height',label="Canvas Height", minimum=256, maximum=1024, value=512, step=64)
